...
import {
  Course, CourseStatus,
  COURSE_NEXT_LABEL, COURSE_NEXT_ICON,
  COURSE_CANCELLABLE, COURSE_STATUS_LABEL, courseWhatsAppUrl,
} from '../../core/models/course.model';
import { CourseService } from '../../core/services/course.service';
import { AddCourseDialogComponent } from '../chauffeurs/add-course-dialog.component';
import { SelectDriverDialogComponent } from '../chauffeurs/select-driver-dialog.component';
...
@Component({
  ...
  template: `
    <div class="page">
      <div class="page-header">
        ...
        <button mat-raised-button color="primary" (click)="openAddCourse()">
        ...
        @if (list.length === 0) {
          ...
          <table mat-table [dataSource]="list" class="mat-elevation-z2">
            ...
            <ng-container matColumnDef="chauffeur">
              ...
              <td mat-cell *matCellDef="let c">
                @if (c.driverId) {
                  <div class="driver-name">{{ c.driverName ?? 'Sans nom' }}</div>
            ...
            <ng-container matColumnDef="actions">
              ...
              <td mat-cell *matCellDef="let c">
                <div class="action-btns">
                  ...
                  @if (nextLabel(c.status); as label) {
  ...
  styles: [`
    ...
    table { width: 100%; min-width: 780px; border-radius: 12px; overflow: hidden; }
...
export class CourseListComponent {
  ...
  async advance(course: Course): Promise<void> {
    // Course non assignée qui passe à "accepter" : il faut d'abord choisir
    // le chauffeur qui a accepté avant de pouvoir transitionner.
    if (course.status === 'en_cours' && !course.driverId) {
      const selected = await firstValueFrom(
        this.dialog
          .open<SelectDriverDialogComponent, unknown, Driver>(SelectDriverDialogComponent, {
            width: '95vw',
            maxWidth: '420px',
          })
          .afterClosed(),
      ...
      if (!selected) return;
      await this.runTransition(course, 'accepter', selected.uid);
      return;
    }

    const next = { en_cours: 'accepter', accepter: 'en_route', en_route: 'a_bord', a_bord: 'terminer' } as const;
    const nextStatus = next[course.status as keyof typeof next];
    if (nextStatus) await this.runTransition(course, nextStatus);
  }

  async cancel(course: Course): Promise<void> {
    await this.runTransition(course, 'annulee');
  }

  private async runTransition(course: Course, status: CourseStatus, driverId?: string): Promise<void> {
    this.busy.set(course.id);
    try {
      const { commission } = await this.svc.updateStatus(course.id, status, driverId);
      this.snack.open(
